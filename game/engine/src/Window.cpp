...
namespace GEngine {
  ...
  void Window::setInputCallback(std::function<void(int, int, int, int)> callback) {
    inputCallback = std::move(callback);
  }

  void Window::underlyingCallback(GLFWwindow*, int key, int scanCode, int action, int mods) {
    if (inputCallback)
